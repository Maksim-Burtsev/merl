...
Ctrl+Z and Ctrl+Y undo and redo, per file, for the whole session, as in a VS Code tab that stays
open: `d` to a definition and `[` back, and the edit made before is still one Ctrl+Z away. A run
of keystrokes on one line is one step, as in VS Code. A reload from disk is one step too, as in
VS Code and Vim: after an agent writes the open file, Ctrl+Z takes back what it wrote, line
endings included, and then your own edits before it; a file written while you were in another
one comes back with that write as the step on top. The edits Ctrl+R drops are one Ctrl+Z away. There is no save step: edits reach the disk `autosave_delay_ms` after the last keystroke, and
