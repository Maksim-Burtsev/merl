...
extension under the type it extends; PHP's functions, methods, types and `const`s, behind
...
already a row), a Swift `let`, `var`, `init` or `enum` case, a PHP property, `enum` case or
`define()`, a Ruby
